# pipegenie/regression/pipegenie_regressor.py
...
import os
from typing import TYPE_CHECKING, Optional, Union # Add Union
...
class PipegenieRegressor(BaseClassificationRegression):
    ...
    def __init__(
        ...
        *,
        ...
        outdir: str = "results",
        n_jobs: int = 1, # Corrected type from str to int
        verbose: bool = True, # Added

        # --- METALEARNING PARAMETERS ---
        enable_metalearning: bool = False,
        save_to_metabase: bool = True,
        metabase_path: Optional[str] = "metabase_test",
        metalearning_seeding_strategy: Union[int, float] = 0.25, # Added
        # --------------------
        ensemble_file_format: str = 'txt', # <-- ADD
    ):
        super().__init__(
            grammar,
            ...
            outdir=outdir,
            n_jobs=n_jobs,
            verbose=verbose, # Added

            # --- PASS NEW PARAMS TO SUPER ---
            enable_metalearning=enable_metalearning,
            save_to_metabase=save_to_metabase,
            metabase_path=metabase_path,
            metalearning_seeding_strategy=metalearning_seeding_strategy, # Added
            # ------------------------------

            ensemble_file_format=ensemble_file_format, # <-- PASS
        )

    def fit(self, X: 'ArrayLike', y: 'ArrayLike', **kwargs) -> 'PipegenieRegressor':
        """
