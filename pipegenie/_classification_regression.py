...
from pipegenie.utils import serialize_estimator # Add this import
...
class BaseClassificationRegression(BasePipegenie, ABC):
    ...
    def _generate_ensemble(self, data: 'dict[str, ArrayLike]') -> None:
        ...
        # --- NEW LOGIC FOR ENSEMBLE FILE ---
        ensemble_fitness = np.mean(fitness_values)

        if self.ensemble_file_format == 'json':
            ensemble_data = {
            ...
            for idx, (name, est) in enumerate(estimators):
                member_data = {
                    "name": name,
                    "fitness": self.elite[idx].fitness.values[0],
                    "pipeline_string": str(self.elite[idx]),
                    "pipeline_json": serialize_estimator(est)
                }
                ensemble_data["members"].append(member_data)
            
            with self.outdir_path.joinpath("ensemble.json").open("w", encoding="utf-8") as log:
                json.dump(ensemble_data, log, indent=4)

        else: # Default to 'txt'
            with self.outdir_path.joinpath("ensemble.txt").open("w", encoding="utf-8") as log:
                for idx, (name, est) in enumerate(estimators):
                    log.write(f"{name}: {str(self.elite[idx])} -> Fitness: {self.elite[idx].fitness.values[0]}\n\n")

                log.write(f"Ensemble fitness: {ensemble_fitness}\n")
