...
class SyntaxTreePipeline(SyntaxTree):
    ...
    def _get_term_str(
        ...
        # --- THE FIX: Check for arity to identify an algorithm ---
        # An algorithm node will have an arity > 0, whether it's a real Primitive
        # or our rehydrated dummy object.
        is_algorithm = hasattr(term.code, 'arity') and term.code.arity > 0

        if is_algorithm:
            term_str = term.symbol + "("
            term_index += 1
            # The arity tells us how many arguments to parse
            num_args_to_parse = term.code.arity

            while (term_index < len(terms) and
                   term_index - start_index < num_args_to_parse + index_offset):
                arg_str, term_index, new_offset = self._get_term_str(
                    terms,
                    term_index,
                    index_offset,
                )
                term_str += arg_str + ","
                # The offset is updated if a nested algorithm is found
                if new_offset > 0:
                    index_offset += new_offset -1 # Subtract 1 because it counts as one argument

            return term_str.rstrip(',') + ")", term_index, num_args_to_parse

        # --- Hyper-parameter logic ---
        # If it's not an algorithm, it must be a hyperparameter.
        # Its value is stored in the .name attribute of its code object.
        if term.code and hasattr(term.code, 'name'):
        ...
        if not is_bool(arg_val) and not is_number(arg_val) and not is_tuple(arg_val) and arg_val != 'None':
            arg_val = "'" + arg_val + "'"

        return str(arg_val), term_index + 1, index_offset
